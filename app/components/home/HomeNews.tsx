...
type TabId = 'tft' | 'steam'
type HomePatchNote = { id: string; title: string; summary: string; publishedAt: string; sourceUrl: string | null }
const TABS: { id: TabId; label: string }[] = [{ id: 'tft', label: '롤체 패치 노트' }, { id: 'steam', label: 'Steam 할인' }]
...
export default function HomeNews({ patchNotes, deals }: { patchNotes: HomePatchNote[]; deals: SteamFeaturedDeal[] | null }) {
  ...
  return <section className={`${CARD} h-fit overflow-hidden`} aria-labelledby="home-news-title">
    ...
    <div className="p-4 sm:p-5">
      ...
      <div id={`${idPrefix}-tft-panel`} role="tabpanel" aria-labelledby={`${idPrefix}-tft-tab`} hidden={activeTab !== 'tft'}>
        ...
        <Link href="/tft" className="mt-4 inline-flex min-h-10 items-center text-xs font-black text-brand-ink hover:underline">롤체 패치 노트 전체 보기 →</Link>
      </div>
      <div id={`${idPrefix}-steam-panel`} role="tabpanel" aria-labelledby={`${idPrefix}-steam-tab`} hidden={activeTab !== 'steam'}>
        ...
        <Link href="/steam" className="mt-4 inline-flex min-h-10 items-center text-xs font-black text-brand-ink hover:underline">Steam 할인 전체 보기 →</Link>
      </div>
    </div>
  </section>
}
