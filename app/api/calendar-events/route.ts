...
export async function GET(request: NextRequest) {
  ...
  const [onceResult, yearlyResult, customResult, memberResult] = await Promise.all([oneTime, yearly, customGames, memberOptions])
  ...
  const events = [...calendarEvents, ...gameEvents].sort((a, b) => Number(a.event_day) - Number(b.event_day) || String(a.event_time ?? '').localeCompare(String(b.event_time ?? '')) || String(a.title).localeCompare(String(b.title), 'ko') || String(a.id).localeCompare(String(b.id)))
