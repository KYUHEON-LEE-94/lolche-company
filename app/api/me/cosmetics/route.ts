...
export async function GET(){
 ...
 const [account,frames,effects,fi,ei,ledger,member]=await Promise.all([
  ...
  supabaseAdmin.from('ranking_card_effects').select('id,key,label,description,effect_key,price_points,is_purchasable,sort_order').eq('is_active',true).order('sort_order'),
  ...
  supabaseAdmin.from('members').select('profile_frame_path,ranking_card_effect_key').eq('id',mine.member.id).single(),
 ])
 const error=account.error??frames.error??effects.error??fi.error??ei.error??ledger.error??member.error
 if(error){
  if(isMissingTableError(error)||isMissingColumnError(error)){ const legacy=await supabaseAdmin.from('profile_frames').select('id,key,label,image_path,sort_order').eq('is_active',true).order('sort_order'); return NextResponse.json({migration_required:true,balance:0,isAdmin:admin,frames:(legacy.data??[]).map((f)=>({...f,price_points:0,is_purchasable:true,owned:true,equipped:false})),effects:[],ledger:[]},{headers:H}) }
