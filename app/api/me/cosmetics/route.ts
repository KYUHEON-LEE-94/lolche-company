...
export async function GET(){
 ...
 // 카드 테마(profile_card_theme_key) 꾸미기는 제거됨 — 랭킹 배경과 역할이 겹쳐 UI/API 노출을 끊었다.
 // DB 테이블/컬럼은 비파괴 원칙상 유지하되, 여기서 더는 조회·노출하지 않는다.
 const [account,frames,effects,fi,ei,ledger,member]=await Promise.all([
  ...
  supabaseAdmin.from('members').select('profile_frame_path,ranking_card_effect_key,ranking_card_bg_image').eq('id',mine.member.id).single(),
 ])
