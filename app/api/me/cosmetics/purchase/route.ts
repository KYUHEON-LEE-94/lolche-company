...
export async function POST(req:Request){
 ...
 if(!isRecord(body)||Object.keys(body).some(k=>!['itemType','itemId'].includes(k))||(body.itemType!=='frame'&&body.itemType!=='rank_effect')||typeof body.itemId!=='string'||!UUID_RE.test(body.itemId))return NextResponse.json({error:'상품 정보가 올바르지 않습니다.'},{status:400})
 const rpc=body.itemType==='frame'?'purchase_profile_frame':'purchase_rank_effect'; const args=body.itemType==='frame'?{p_member_id:mine.member.id,p_frame_id:body.itemId}:{p_member_id:mine.member.id,p_effect_id:body.itemId}
